...
(define (breakpoint exn)
  ...
  (if (exn:break? exn)
    ...
    (ftal^ exn))) ; From call-with-exception docs: —an exception handler can simply return a result instead of escaping, in which case the raise call propagates the value to the previous exception handler
